...
jQuery('.cart__items-wrapper,.cart__footer-wrapper').wrapAll('<div class="container padding-top"><div class="row"></div></div>');
jQuery('.carthead').prependTo('.template-cart .wrapper > .container > .row');
jQuery('.main-home-section').prependTo('.wrapper');
jQuery('.half-ring-design,.card-sec2,.service-tag-div,.best-product-sec').wrapAll('<div class="main-top"></div>');
jQuery('.pro-card-sec,.our-pro-shop-sec,.logo-slider').wrapAll('<div class="bg-style main1"></div>');
// jQuery('.main-top').insertAfter('.main-home-section');
jQuery('.ring-img,.card-sec1,.testimonials-sec,.blog-sec').wrapAll('<div class="main-bottom"></div>');
jQuery('.main-bottom').appendTo('.wrapper');
$(document).ready(function () {
  /********* desktop menu *********/
